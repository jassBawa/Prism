...
/** Fetch + decode every basket on the program (getProgramAccounts).
 * Filter by the exact account size so an orphaned pre-upgrade `basket-v2`
 * account (a smaller, differently-shaped Basket with the same discriminator)
 * can't crash the borsh decoder. */
export async function fetchAllBaskets(program: Program<MiniSymmetry>): Promise<BasketView[]> {
  const all = await program.account.basket.all([{ dataSize: program.account.basket.size }]);
  return all
    .map(({ publicKey, account }) => {
      const a = account as unknown as RawBasket;
      const n = a.numAssets;
