...
async function main() {
  ...
  for (const b of cfg.baskets) {
    ...
    console.log(
      "   " +
        b.assets
          .map((a, i) => `${a.key.toUpperCase()} ${((weightsBps[i] ?? 0) / 100).toFixed(0)}%→${(a.weightBps / 100).toFixed(0)}%`)
          .join("   "),
    );
    console.log(
      `   NAV $${(navMicro / 1e6).toFixed(2)} | supply ${supply.uiAmount} | drift ${(maxDriftBps / 100).toFixed(2)}% (thr ${(acct.rebalanceThresholdBps / 100).toFixed(2)}%) | paused ${acct.paused}\n`,
    );
  }
}
