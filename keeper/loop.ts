...
async function topUpReserves(
  ...
  for (const b of baskets)
    for (const a of b.assets)
      if (!seen.has(a.mint)) seen.set(a.mint, a.decimals);
  for (const [mint, decimals] of seen) {
    try {
      ...
      if (bal < min) {
        const amt =
          BigInt(TARGET_RESERVE_WHOLE) * 10n ** BigInt(decimals) - bal;
        await mintTo(conn, admin, pk(mint), reserve, admin, amt);
        console.log(
          `  reserve top-up: ${mint.slice(0, 6)} → ~${TARGET_RESERVE_WHOLE}`,
        );
      }
    } catch {
...
async function tickBasket(
  ...
  const drift = computeDrift(balances, pricesMicro, b.assets);
  const { navMicro, weightsBps, maxDriftBps } = drift;
  const maxRelBps = drift.relDriftBps.length
    ? Math.max(...drift.relDriftBps)
    : 0;
  ...
  console.log(
    `[${label}] nav=$${(navMicro / 1e6).toFixed(2)} w=[${weightsBps.map((w) => (w / 100).toFixed(0)).join("/")}]% ` +
      ...
      `(thr ${(b.thresholdBps / 100).toFixed(2)}%/${(b.thresholdRelBps / 100).toFixed(2)}%) ` +
      `spread=${(b.spreadBps / 100).toFixed(2)}% elapsed=${elapsed}s`,
  );
  if (
    navMicro === 0 ||
    elapsed < b.intervalSecs ||
    !driftTriggers(drift, b.thresholdBps, b.thresholdRelBps)
  )
    return;
  ...
  const sigs = await sendWithPyth(
    ...
    (priceFor) =>
      program.methods
        ...
        .remainingAccounts(
          rebalanceRemaining(b.pubkey, keeper, b.assets, priceFor),
        )
        .instruction(),
  );
...
export async function startKeeper(opts: KeeperOpts = {}): Promise<void> {
  ...
  console.log(
    `keeper ${opts.once ? "(single tick)" : `polling every ${pollMs}ms`} | RPC ${process.env.RPC_URL ?? "devnet"}`,
  );
  do {
    try {
